...
% end
% 
% for m= 2:size(size(s7_low,2))           %2차 미분
% 
...
div=sort(div);
div=unique(div);
s4_low = zeros(size(s2_low));   %나눈창에대한 문턱처리 저장

for i = 1: length(div)-1

th2=max(max(s3_low(:,div(i)+1:div(i+1))))*TH2;  %각각의 창에 대한 문턱값.
s4_low(:,div(i)+1:div(i+1))=(s3_low(:,div(i)+1:div(i+1))>th2).*s3_low(:,div(i)+1:div(i+1));

end

th2=max(max(s3_low(:,div(end)+1:end)))*TH2;  %각각의 창에 대한 문턱값.
